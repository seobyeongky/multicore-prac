	...
	/*!< in: id of the instance */
{
    ...
    buf_pool_t* buf_pool = param->buf_pool;
    ulint buf_pool_size = param->buf_pool_size;
    ulint instance_no = param->instance_no;
	...
	ulint	n_instances)	/*!< in: number of instances */
{
	...
	for (i = 0; i < n_instances; i++) {
        ...
        param.buf_pool = ptr;
        param.buf_pool_size = size;
        param.instance_no = i;

        // make threads and go concurrently
        if (pthread_create(&threads[i], 0, buf_pool_init_instance, (void*)&param) < 0) {
            // MAKE ERROR
            return(DB_ERROR);
        }
	}
	...
	if (srv_numa_interleave) {
		ib_logf(IB_LOG_LEVEL_INFO,
			"Setting NU
            MA memory policy to MPOL_DEFAULT");
